...
const FruitTypeSelection = () => {
  ...
  const uniqueValues = useMemo(() => {
    const values = [...new Set(filteredData.map(([, details]) => details['Fruit type']))].filter(value => value);
    return values;
  }, [filteredData]);
  ...
  const handleValueSelect = (value) => {
    ...
    const filteredForNextStep = filteredData.filter(
      ([, details]) => details['Fruit type'] === value
    );
  ...
  const handleNext = () => {
    if (selectedValue) {
      const filteredForNextStep = filteredData.filter(
        ([, details]) => details['Fruit type'] === selectedValue
      );
      navigate('/fruit-shape-selection', { state: { filteredData: filteredForNextStep } });
    } else {
  ...
  const handleShowPossibleData = () => {
    if (selectedValue) {
      ...
      alert('Please select a Fruit Type first.');
    }
  };
...
export default FruitTypeSelection;
