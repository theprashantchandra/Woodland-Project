...
const FruitColourSelection = () => {
  ...
  const uniqueValues = useMemo(() => {
    const values = [...new Set(filteredData.map(([, details]) => details['Fruit colour']))].filter(value => value);
    return values;
  }, [filteredData]);
  ...
  const handleValueSelect = (value) => {
    ...
    const filteredForNextStep = filteredData.filter(
      ([, details]) => details['Fruit colour'] === value
    );
  ...
  const handleNext = () => {
    if (selectedValue) {
      const filteredForNextStep = filteredData.filter(
        ([, details]) => details['Fruit colour'] === selectedValue
      );
      navigate('/final-leaf-selection', { state: { finalFilteredData: filteredForNextStep } });
    } else {
      alert('Please select a Fruit Colour.');
    }
  };
  ...
  const handleShowPossibleData = () => {
    if (selectedValue) {
      ...
      alert('Please select a Fruit Colour first.');
    }
  };
...
export default FruitColourSelection;
