...
const BarkColourSelection = () => {
  ...
  const uniqueValues = useMemo(() => {
    const values = [...new Set(filteredData.map(([, details]) => details['Bark Colour']))].filter(value => value);
    return values;
  }, [filteredData]);
  ...
  const handleValueSelect = (value) => {
    ...
    const filteredForNextStep = filteredData.filter(
      ([, details]) => details['Bark Colour'] === value
    );
  ...
  const handleNext = () => {
    if (selectedValue) {
      const filteredForNextStep = filteredData.filter(
        ([, details]) => details['Bark Colour'] === selectedValue
      );
      navigate('/final-leaf-selection', { 
        state: { 
          finalFilteredData: filteredForNextStep, 
          selectedAttributes: { ...(location.state?.selectedAttributes || {}), 'Bark Colour': selectedValue } 
        } 
      });
    } else {
      alert('Please select a Bark Colour.');
    }
  };
  ...
  const handleShowPossibleData = () => {
    if (selectedValue) {
      ...
      alert('Please select a Bark Colour first.');
    }
  };
...
export default BarkColourSelection;
