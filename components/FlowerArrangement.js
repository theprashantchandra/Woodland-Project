...
const FlowerArrangement = () => {
  ...
  const uniqueValues = useMemo(() => {
    const values = [...new Set(Object.values(treeSpeciesData).map(details => details['Flower arrangement']))].filter(value => value);
    return values;
  }, []);

  const handleValueSelect = (value) => {
    setSelectedValue(value);
    const filteredForNextStep = Object.entries(treeSpeciesData).filter(
      ([, details]) => details['Flower arrangement'] === value
    );
  ...
  const handleNext = () => {
    if (selectedValue) {
      const filteredForNextStep = Object.entries(treeSpeciesData).filter(
        ([, details]) => details['Flower arrangement'] === selectedValue
      );
      navigate('/flower-colour', { state: { filteredData: filteredForNextStep } });
    } else {
  ...
  const handleShowPossibleData = () => {
    if (selectedValue) {
      const filteredPossibleData = Object.entries(treeSpeciesData).filter(
        ([, details]) => details['Flower arrangement'] === selectedValue
      );
      setPossibleData(filteredPossibleData);
      setOpen(true);
    } else {
      alert('Please select a Flower arrangement first.');
    }
  };
    ...
    <div>
      ...
      <div style={{ padding: isSmallScreen ? '10px' : '20px' }}>
        <Typography variant={isSmallScreen ? "h5" : "h4"} align="center" gutterBottom>
          Please select Flower Arrangement
        </Typography>
